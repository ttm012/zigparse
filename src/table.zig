const std = @import("std");

/// Parses CSV/TSV data and prints a formatted table.
///
/// Features:
///   - Quoted fields with embedded delimiters and newlines
///   - Escaped quotes ("")
///   - Header row highlighted in cyan
///   - BOM (UTF-8) handling

pub fn parse(alloc: std.mem.Allocator, data: []const u8, delimiter: u8) !void {
    // Strip UTF-8 BOM if present
    const content = if (std.mem.startsWith(u8, data, "\xEF\xBB\xBF"))
        data[3..]
    else
        data;

    var row: u32 = 0;
    var col: u32 = 0;
    var in_quotes = false;
    var cell = std.ArrayList(u8).init(alloc);
    defer cell.deinit();

    var i: usize = 0;
    while (i < content.len) : (i += 1) {
        const c = content[i];

        if (in_quotes) {
            if (c == '"') {
                // Escaped quote: ""
                if (i + 1 < content.len and content[i + 1] == '"') {
                    try cell.append('"');
                    i += 1;
                } else {
                    in_quotes = false;
                }
            } else {
                try cell.append(c);
            }
        } else {
            switch (c) {
                '"' => in_quotes = true,
                '\n', '\r' => {
                    if (cell.items.len > 0 or col > 0) {
                        printCell(cell.items, row);
                        std.debug.print("\n", .{});
                        row += 1;
                    }
                    cell.clearRetainingCapacity();
                    col = 0;
                    // Handle \r\n
                    if (c == '\r' and i + 1 < content.len and content[i + 1] == '\n') {
                        i += 1;
                    }
                },
                else => {
                    if (c == delimiter) {
                        printCell(cell.items, row);
                        cell.clearRetainingCapacity();
                        col += 1;
                    } else {
                        try cell.append(c);
                    }
                },
            }
        }
    }

    // Last cell (file may not end with newline)
    if (cell.items.len > 0 or col > 0) {
        printCell(cell.items, row);
        row += 1;
    }

    std.debug.print("  {d} rows\n", .{row});
}

fn printCell(cell: []const u8, row: u32) void {
    if (row == 0) {
        // Header row: bold cyan
        std.debug.print("\x1b[1;36m{s}\x1b[0m \x1b[90m│\x1b[0m ", .{cell});
    } else {
        // Data rows: plain
        std.debug.print("{s} \x1b[90m│\x1b[0m ", .{cell});
    }
}
